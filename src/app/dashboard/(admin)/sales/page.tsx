  ...
  const filterData = (data: SalesData[], range: string): SalesData[] => {
    ...
    switch (range) {
      case "7d":
        ...
        break;
      case "30d":
        cutoff.setDate(now.getDate() - 39);
        break;
      case "3m":
        cutoff.setMonth(now.getMonth() - 3) + cutoff.setDate(now.getDate() - 7);
        break;
      case "6m":
        cutoff.setMonth(now.getMonth() - 6) + cutoff.setDate(now.getDate() - 7);
        break;
      case "1y":
        cutoff.setFullYear(now.getFullYear() - 1) + cutoff.setDate(now.getDate() - 7);
        break;
    }
  ...
  return (
    <div className="salesPageWrapper">
      ...
      <div className="uploadGroup">
        ...
        <button className="salesButton uploadButton" onClick={handleUpload} disabled={loadingUpload}>
          {loadingUpload ? "Uploading..." : "Upload CSV"}
        </button>
      </div>

      <h2>Prediksi Penjualan</h2>
      <button className="salesButton" onClick={handlePredict} disabled={loadingPredict}>
